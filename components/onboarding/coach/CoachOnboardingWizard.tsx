...
import Step3Certifications from "./steps/Step3Certifications";
import Step4JoinAcademy from "./steps/Step4JoinAcademy";
...
export type CoachFormData = {
  ...
  // Step 2
  coachingRole: string; specialisations: string[]; ageGroupsCoached: string[];
  coachingExperienceYears: number; city: string; state: string;
  playedStateLevel: string; highestLevelPlayed: string; playingRole: string;
  // Step 3
  highestCertification: string; certificateUrl: string;
  // Step 4
  joinMethod: string; inviteCode: string; inviteVerified: boolean;
  inviteAcademyId: string; inviteAcademyName: string; inviteAcademyCity: string;
  academySearchCity: string; academySearchState: string;
};

const INIT: CoachFormData = {
  phone: "", otp: ["","","","","",""], otpSent: false, otpVerified: false,
  firstName: "", lastName: "", email: "",
  coachingRole: "", specialisations: [], ageGroupsCoached: [],
  coachingExperienceYears: 0, city: "", state: "",
  playedStateLevel: "", highestLevelPlayed: "", playingRole: "",
  highestCertification: "", certificateUrl: "",
  joinMethod: "invite", inviteCode: "", inviteVerified: false,
  inviteAcademyId: "", inviteAcademyName: "", inviteAcademyCity: "",
  academySearchCity: "", academySearchState: "",
};

const LS_KEY = "coach-onboarding-draft";

const STEPS = [
  { lbl: "Account",      sub: "Phone + identity" },
  { lbl: "Experience",   sub: "Role, skills, history" },
  { lbl: "Certifications", sub: "BCCI / NCA level" },
  { lbl: "Join Academy", sub: "Code or browse" },
];

// ── Validation ────────────────────────────────────────────────────────────────
function validate(step: number, data: CoachFormData): Record<string, string> {
  ...
  if (step === 1) {
    if (!data.coachingRole)                   errs.coachingRole     = "Select your coaching role.";
    if (!data.specialisations.length)         errs.specialisations  = "Select at least one specialisation.";
    if (!data.ageGroupsCoached.length)        errs.ageGroupsCoached = "Select at least one age group.";
    if (!data.city.trim())                    errs.city             = "City is required.";
    if (!data.state)                          errs.state            = "State is required.";
  }
  if (step === 2) {
    if (!data.highestCertification)           errs.highestCertification = "Select your certification level.";
  }
  // Step 3 (JoinAcademy) is optional — no required fields
  return errs;
}

// ── CSS (matches Academy v3 tokens exactly, scoped under .cow-root) ───────────
const CSS = `
:root{
...
.cow-root *,.cow-root *::before,.cow-root *::after{box-sizing:border-box;}
.cow-root{font-family:"Barlow",system-ui,sans-serif;-webkit-font-smoothing:antialiased;min-height:100vh;min-height:100dvh;background:var(--bg);color:var(--text);}
.cow-root a{color:var(--accent-bright);text-decoration:none;}
...
/* academy results list */
.academy-list{display:flex;flex-direction:column;gap:.5rem;margin-top:.8rem;}
.academy-row{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.85rem 1rem;border-radius:9px;background:var(--field-bg);border:1px solid var(--card-border);}
.academy-row .aname{font-weight:700;font-size:.95rem;}
.academy-row .ameta{font-size:.78rem;color:var(--text-dim);margin-top:.15rem;}
.academy-row .applied{font-size:.8rem;color:var(--ok);font-weight:600;}

/* cert cards */
.cert-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:.55rem;}
.cert-card{cursor:pointer;border-radius:10px;padding:.8rem .9rem;background:var(--field-bg);border:1.5px solid var(--card-border);transition:all .18s var(--ease);text-align:left;width:100%;}
.cert-card:hover{border-color:rgba(245,245,240,.3);}
.cert-card[aria-pressed="true"]{border-color:var(--accent);background:var(--ov14);}
.cert-card .badge{display:inline-block;padding:.18rem .55rem;border-radius:5px;font-family:"Barlow Semi Condensed",sans-serif;text-transform:uppercase;letter-spacing:.08em;font-size:.72rem;font-weight:700;background:var(--ov14);color:var(--accent-bright);margin-bottom:.35rem;}
.cert-card[aria-pressed="true"] .badge{background:var(--accent);color:#1a0e02;}
.cert-card .ct{font-size:.9rem;font-weight:700;color:var(--text);}
.cert-card .cs{font-size:.73rem;color:var(--text-dim);margin-top:.1rem;}
...
@media(max-width:560px){
  .grid2,.cardset,.cert-cards{grid-template-columns:1fr;}
...
// ── Wizard ────────────────────────────────────────────────────────────────────
export default function CoachOnboardingWizard({ initialStep = 0 }: { initialStep?: number }) {
  ...
  const patch = useCallback((partial: Partial<CoachFormData>) => {
    setForm((prev) => {
  ...
  async function handleNext() {
    if (done) { router.push("/dashboard/coach"); return; }
    ...
    if (step === 3) {
      setFinishing(true);
      try {
        await fetch("/api/onboarding/coach/complete", {
          method: "PATCH", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            firstName: form.firstName, lastName: form.lastName, email: form.email,
            coachingRole: form.coachingRole, specialisations: form.specialisations,
            ageGroupsCoached: form.ageGroupsCoached,
            coachingExperienceYears: form.coachingExperienceYears,
            city: form.city, state: form.state,
            playedStateLevel: form.playedStateLevel === "Yes",
            highestLevelPlayed: form.highestLevelPlayed, playingRole: form.playingRole,
            highestCertification: form.highestCertification, certificateUrl: form.certificateUrl,
            inviteCode: form.inviteVerified ? form.inviteCode : null,
            inviteAcademyId: form.inviteVerified ? form.inviteAcademyId : null,
          }),
        });
    ...
    await fetch("/api/onboarding/coach/step/" + (step + 1), {
  ...
  const pct = done ? 100 : ((step + 1) / 4) * 100;
  const stepProps = { data: form, patch, errors, clearError };

  return (
    <div className="cow-root">
      ...
      <div className="ob">
        ...
        <aside className="rail">
          ...
          <div className="lead">
            ...
            <h1>Build your <b>coaching</b> profile.</h1>
            <p>Four steps. Start evaluating players and linking to academies in under 10 minutes.</p>
          </div>
        ...
        <main className="form-side">
          <div className="topbar">
            <div className="roleswitch">
              ...
              <button type="button" aria-selected={true  as unknown as boolean | undefined}>Coach</button>
              <button type="button" aria-selected={false as unknown as boolean | undefined}>Academy</button>
            ...
            <div className="form-body">
              {done ? (
                <div className="done-screen">
                  ...
                  <h2>{form.firstName || "Coach"}, you&apos;re <b>ready.</b></h2>
                  <p>Your coach profile is live. Link to an academy from your dashboard to start evaluating players.</p>
                </div>
              ) : (
                <>
                  {step === 0 && <Step1Account {...stepProps} />}
                  {step === 1 && <Step2Experience {...stepProps} />}
                  {step === 2 && <Step3Certifications {...stepProps} />}
                  {step === 3 && <Step4JoinAcademy {...stepProps} />}
                </>
              )}
            </div>
          </div>
          ...
          <div className="form-foot">
            <div className="meta">{done ? "Done" : `Step ${step + 1} of 4`}</div>
            <div className="btns">
              {!done && step > 0 && (
