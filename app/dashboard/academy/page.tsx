...
export default async function AcademyDashboardPage() {
  ...
  if (!session?.user) redirect("/auth/login?from=/dashboard/academy");
  if ((session.user as any).role !== "academy_admin") redirect("/dashboard");
  const adminId = (session.user as any).id as string;

  const aRows = await safe(
    sql`SELECT id, academy_name, profile_status, verification_status FROM academies WHERE user_id = ${adminId} LIMIT 1` as unknown as Promise<any[]>,
    [] as any[],
  );
  const academy = (aRows as any[])[0];
  ...
  const [
    ...
    safe(sql`
      SELECT AVG(NULLIF(ba.mindset_score, 0))::float AS avg_mindset
    ...
    safe(sql`
      SELECT pp.user_id, pp.id AS player_profile_id,
  ...
  return (
    <AcademyDashboardClient
      ...
      verificationStatus={academy.verification_status ?? "PENDING"}
      counts={(countsRows as any[])[0]}
      scoutViews30d={(viewsRows as any[])[0]?.cnt ?? 0}
      scoutSaves={(savesRows as any[])[0]?.cnt ?? 0}
      invites={(invitesRows as any[])[0]}
      fitness={(fitnessRows as any[])[0]}
      behavioural={(behavRows as any[])[0]}
      topPerformers={topRows as any[]}
      recentPlayers={recentRows as any[]}
    />
  );
}
